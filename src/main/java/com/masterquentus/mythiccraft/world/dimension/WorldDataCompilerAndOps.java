...
// snatched from twilight forest
// seems to be like in place data gen for the dimensions
// probably dont mess with it and everything will work out alright
...
public abstract class WorldDataCompilerAndOps<Format> extends WorldGenSettingsExport<Format> implements IDataProvider {
    ...
    public WorldDataCompilerAndOps(DataGenerator generator, DynamicOps<Format> delegate, Function<Format, String> fileContentWriter, DynamicRegistries dynamicRegistries) {
        super(delegate, dynamicRegistries);
    ...
    @Override
    public final void run(final DirectoryCache directoryCache) {
    ...
    private <Resource> Optional<ResourceLocation> rummageForResourceLocation(Resource resource, RegistryKey<? extends Registry<Resource>> registryKey) {
        ...
        if (!instanceKey.isPresent()) {
            try {
                ...
                instanceKey = dynRegistry != null ? dynRegistry.getResourceKey(resource).map(RegistryKey::location) : Optional.empty();
            } catch (Throwable t) {
        ...
        if (!instanceKey.isPresent()) {
            Registry<Resource> registry = getFromVanillaRegistryIllegally(WorldGenRegistries.REGISTRY, registryKey);

            if (registry != null) {
                instanceKey = registry.getResourceKey(resource).map(RegistryKey::location);
            }
        }
    ...
    protected <Resource> DataResult<Format> encode(Resource resource, Format dynamic, RegistryKey<? extends Registry<Resource>> registryKey, Codec<Resource> codec) {
        ...
        if (instanceKey.isPresent()) {
            if (MythicCraft.MOD_ID.equals(instanceKey.get().getNamespace())) // This avoids generating anything that belongs to Minecraft
