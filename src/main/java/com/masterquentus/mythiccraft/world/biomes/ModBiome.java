...
public class ModBiome {
	public Biome create(Biome.Builder biome) {
		return biome.generationSettings(this.getGenSettings().build()).mobSpawnSettings(this.getMobSpawns().build())
				.temperatureAdjustment(Biome.TemperatureModifier.NONE).specialEffects(this.getEffects()).build();
	}

	// TODO: use this to set the right water colors
	protected BiomeAmbience getEffects() {
		return new BiomeAmbience.Builder().waterColor(4159204).waterFogColor(329011).fogColor(12638463)
				.skyColor(calculateSkyColor(0.95F)).ambientMoodSound(MoodSoundAmbience.LEGACY_CAVE_SETTINGS).build();
	}
	...
	protected BiomeGenerationSettings.Builder getGenSettings() {
		BiomeGenerationSettings.Builder worldGenSettings = new BiomeGenerationSettings.Builder()
				.surfaceBuilder(getSurface());
