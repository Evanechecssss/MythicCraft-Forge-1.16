...
public class VampireVindicatorEntity extends VindicatorEntity implements IAnimatable, IInfectingable {
	...
	public static AttributeModifierMap.MutableAttribute createAttributes() {
		return MonsterEntity.createMonsterAttributes().add(Attributes.MOVEMENT_SPEED, 1.1D)
				.add(Attributes.FOLLOW_RANGE, 20.0D).add(Attributes.MAX_HEALTH, 48.0D);
	}
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (event.getController().getCurrentAnimation() == null) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation(AnimationTypes.IDLE.registerName, true));
		}
		event.getController().setAnimation(
				new AnimationBuilder().addAnimation(currentAnimation.registerName, currentAnimation.loop));
	...
	private static final DataParameter<Integer> ATTACK_TIME = EntityDataManager.defineId(VampireEvokerEntity.class,
			DataSerializers.INT);
	...
	private enum AnimationTypes {
		IDLE("animation.model.idle", true), WALK("animation.model.walk", true), ATACK("animation.model.atack", false);
