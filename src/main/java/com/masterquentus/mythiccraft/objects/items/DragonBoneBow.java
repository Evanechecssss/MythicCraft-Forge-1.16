...
public class DragonBoneBow extends BowItem {
			...
			if (!itemstack.isEmpty() || flag) {
				...
				if (!((double) f < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						...
						abstractarrowentity.shootFromRotation(playerentity, playerentity.xRot, playerentity.yHeadRot,
								0.0F, f * 3.0F, 1.0F);
						if (f == 1.0F) {
						...
						abstractarrowentity.getPersistentData().putBoolean(MythicCraft.MOD_ID + "kaupenbow", true);
